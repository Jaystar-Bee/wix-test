...
<template>
  <div class="py-20 px-10">
    <div class="mb-6 px-2">
      <h2
        @click="setForm($event, 'title', FormType.TEXT, data?.title, null)"
    ...
    <ul
      class="flex items-stretch gap-5 overflow-x-auto py-4 px-2"
      :class="{
        '': data?.cards?.length > 1,
      }"
    >
      ...
      >
        <div
          v-if="card?.bgImage || card?.button?.text"
        ...
        >
          ...
          <div
            ...
            class="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white rounded-full px-6 py-1.5 text-xs"
            :style="{
              background: card?.button?.bgColor,
            }"
          >
            <p
              :style="{
        ...
        <div class="absolute -top-2 -right-2 text-red-500 text-xl">
          <i
            ...
            aria-hidden="true"
            @click="deleteCard($event, 'cards', index)"
          ></i>
        </div>
      </li>
      <li
