...
<template>
  <Popper reference-is="button" popper-is="span">
    <template #reference>
      <button class="text-white bg-green-600 text-xs px-4 py-1 rounded-md whitespace-nowrap">
        {{ isTop ? "Add to top" : isAdd ? "Add Component" : "Add to bottom" }}
      </button>
    </template>
    <ul class="list-none bg-white shadow-md px-4 py-2 rounded-md max-h-[5rem] overflow-y-auto mt-1">
      <li v-for="(comp, index) in components" @click="addComponent(comp)" :key="comp"
